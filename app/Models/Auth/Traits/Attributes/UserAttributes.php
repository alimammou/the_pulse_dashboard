...
trait UserAttributes
{
    ...
    public function checkAdmin()
    {
        ...
        if ($this->id != 1) {
            $str .= '<div class="btn-group">
                        ...
                        <li>'.$this->getApproveButtonAttribute('').'</li>
    ...
    public function getStatusButtonAttribute(string $class): string
    {
            ...
            switch ($this->status) {
                case UserStatus::Inactive():
                    if (access()->allow('activate-user')) {
                        ...
                        return '<a class="'.$class.'" href="'.route('admin.auth.user.mark', [$this, UserStatus::Active]).'"><i class="fa fa-check-square" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.activate').'"></i>'.$name.'</a>';
                    }
                ...
                case UserStatus::Active():
                    if (access()->allow('deactivate-user')) {
                        ...
                        return '<a class="'.$class.'" href="'.route('admin.auth.user.mark', [$this, UserStatus::Inactive]).'"><i class="fa fa-square" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.access.users.deactivate').'"></i>'.$name.'</a>';
                    }
    ...
    public function getApproveButtonAttribute($class)
    {
        if ($this->id != access()->id() && access()->allow('delete-user') && !$this->approved_at) {
            $name = $class == '' ? 'Approve' : '';
            return '<a class="'.$class.'" href="'.route('admin.auth.user.approve', $this).'"
                 data-method="approve"
                 date-name="approve-item"
                 data-trans-button-cancel="'.trans('buttons.general.cancel').'"
                 data-trans-button-confirm="approve"
                 data-trans-title="Approve '.$this->name .'?"><i class="fa fa-trash" data-toggle="tooltip" data-placement="top" title="approve"></i>'.$name.'</a>';
    ...
    public function getIsNormalUserAttribute(): bool
    {
        return ! $this->is_root;
    }
}
